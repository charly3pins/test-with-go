...
func (us *UserStore) Find(id int) (*User, error) {
	...
	case nil:
		return &user, nil
	default:
		return nil, fmt.Errorf("psql: error querying for user by id, %q", err)
	}
}
...
func (us *UserStore) Create(user *User) error {
	...
	if err != nil {
		return fmt.Errorf("psql: error creating new user")
	}
	return nil
}
...
func (us *UserStore) Delete(id int) error {
	...
	if err != nil {
		return fmt.Errorf("psql: error deleting user, %q", err)
	}
	return nil
}
